...
.factory('socket', ['$rootScope', function($rootScope) {
  var socket = io.connect(sockio_url, {transports: ['websocket']});
...
.controller('PositionCtrl', function($scope, $http, socket, $ionicModal, $location, $rootScope){
 ...
 socket.on('reset_alert', function(data){
  ...
  $scope.toggleEmergency = function(){
    ...
    socket.emit("emergency",{"action":$scope.emergency==true ? 1 : 0});
  ...
  $scope.resetAlarms = function(){
   ...
   socket.on("reset_done", function(message){
  ...
  socket.on("alarms", function(message){
	...
	if(r[1][1] == 1){
			if($scope.fault_img.indexOf("green_l.png")>0){
				$scope.a_modal.show();
			}
		  $scope.almin = "./images/on.png";
	      $scope.fault_img = "./images/red_l.png";
...
.controller('HomeCtrl', function($scope, $http, $state,socket, $location, $ionicModal){
  ...
  socket.on("alarm_error", function(message){
    ...
    if((message.startsWith("No Alarms"))){
		...
		if($scope.display_text.indexOf("STOP JOG")>=0){
		
			if($scope.dir == 1 && message.indexOf("POT Limit Exceeded")>=0){
				$scope.display_text = "START JOG";;
			}
			
			if($scope.dir == 0 && message.indexOf("NOT Limit Exceeded")>=0){
				$scope.display_text = "START JOG";;
			}
		}
  ...
  $http.get(DAC_SETTINGS_API)
     .success(function(data){
       ...
       $scope.max_rpm = data.resp.A.max_rpm;
     })
   .error(function(err){
     //console.log(err);
   })
    ...
    if($scope.mode == "step"){
	  ...
	  if(($scope.dir==0 && gmsg=="NOT Limit Exceeded") || ($scope.dir==1 && gmsg=="POT Limit Exceeded") || ($scope.dir==0 && gmsg=="NOT Hard Limit Switch Latched") || ($scope.dir==1 && gmsg=="POT Hard Limit Switch Latched")){
			//socket.emit("jog_mode",f);
			  ...
			  document.getElementById("refintim").innerHTML = "STEP MODE";
      ...
      if($scope.display_text == "START JOG"){
        if($scope.p.feed_rate > 0 && $scope.p.feed_rate < 25){
		  ...
		  if(($scope.dir==0 && gmsg=="NOT Limit Exceeded") || ($scope.dir==1 && gmsg=="POT Limit Exceeded") || ($scope.dir==0 && gmsg=="NOT Hard Limit Switch Latched") || ($scope.dir==1 && gmsg=="POT Hard Limit Switch Latched")){
			//socket.emit("jog_mode",f);
			  ...
			  socket.emit("jog_mode",f);
			  document.getElementById("refintim").innerHTML =  "START JOG";
        ...
        $scope.display_text = "START JOG";
		document.getElementById("refintim").innerHTML =  "STOP JOG";
      }
    }
  }
})

.controller('SettingsCtrl', function($scope, $state,socket) {
  $scope.p= {}
  $scope.p.passwd = "";
  $scope.err_msg = "";
  ...
  $scope.checkSystemUpdate = function(){
    socket.emit("check_system_update", {"status":""});
  }
  $scope.performSystemUpdate = function(){
    socket.emit("perform_system_update", {"status":""});
  }
})
.controller('AutoCtrl', function($rootScope, $scope, $stateParams, $ionicModal, $state, socket, $http) {
  ...
  socket.emit("exec_next_line", {"data":"1"});
  }
  ...
  $http.get(DAC_SETTINGS_API)
     .success(function(data){
       ...
       if(ecs == 1){
         ...
         socket.emit('set_program_mode',{data:'continuous'})
       }else {
         //Enable both
         $scope.display = true;
       }
  ...
  $scope.setPgmMode = function(mode){
    ...
    if(mode == "s"){
      ...
      $scope.s_mode_button = "mode_button_active";
      $scope.c_mode_button = "mode_button";
      socket.emit("set_program_mode", {data:"single"});
    }
    else{
      ecs = 0;
...
.controller('DriveOffsetCtrl', function($scope, $http, $state, socket, $ionicPopup){
	...
	$http.get(DAC_SETTINGS_API)
     ...
     var keys = ['g54', 'g55', 'g56', 'g57', 'g58', 'g59'];
        ...
        // $scope.params.A[keys[i]] = 0;
			 }else {
        // $scope.params.A[keys[i]]=$scope.params.A[keys[i]];
       }
		 }
     console.log("Update dac settings2")
...
.controller('PitchErrorCtrl', function($scope, $http, $state){
  ...
  $http.get(DAC_SETTINGS_API)
   .success(function(data){
     //console.log(data);
     $scope.rp = data.resp;
     $scope.params = data.resp.A.pitch_error;
